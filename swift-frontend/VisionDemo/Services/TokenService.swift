...
final class TokenService: ObservableObject, Sendable {
    ...
    private let sandboxId: String? = {
        if let value = Bundle.main.object(forInfoDictionaryKey: "LiveKitSandboxId") as? String {
            ...
            print("🔑 Sandbox ID found: \(trimmed)")
            return trimmed
        }
        print("⚠️ No Sandbox ID found in Info.plist")
        return nil
    }()
    ...
    private func fetchConnectionDetailsFromSandbox(roomName: String, participantName: String) async throws -> ConnectionDetails? {
        guard let sandboxId else {
            print("❌ No sandbox ID - cannot fetch connection details")
            return nil
        }
        print("🔵 Fetching connection details with sandbox ID: \(sandboxId)")
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
            print("❌ Failed to connect to LiveKit Cloud sandbox")
            return nil
        }

        print("📡 Sandbox response status: \(httpResponse.statusCode)")
        
        guard (200 ... 299).contains(httpResponse.statusCode) else {
            let responseBody = String(data: data, encoding: .utf8) ?? "Unable to decode response"
            print("❌ Error from LiveKit Cloud sandbox: \(httpResponse.statusCode)")
